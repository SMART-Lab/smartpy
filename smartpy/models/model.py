...
class Model(object):
    ...
    def save(self, savedir="./", hyperparams_filename="hyperparams.json", params_filename="params"):
        #save_dict_to_json_file(pjoin(savedir, hyperparams_filename), self.hyperparams)
        params = {param.name: param.get_value() for param in self.parameters}
        np.savez(pjoin(savedir, params_filename + ".npz"), **params)

    # @classmethod
    # def load(cls, loaddir="./", hyperparams_filename="hyperparams.json", params_filename="params.npz"):
    #     hyperparams = load_dict_from_json_file(pjoin(loaddir, hyperparams_filename))
    #     model = cls(**hyperparams)
    #     params = np.load(pjoin(loaddir, params_filename))
